...
if($data) {
    ...
    foreach($data as $num=>$id):
        if(strlen($id) > 1):
            ...
            $id = trim($id);
            $query = mysql_query("INSERT INTO `$gaSql[db]`.`$sTable` (`id` ,`pair` ,`error`)
							VALUES ('' , '$id', ''); ");
            if($query) { //echo $query;
                $mess .= " ".$id." inserted<br />";
            } else { //echo $query;
...
mysql_query("INSERT INTO `temp` ( `pair` ) SELECT DISTINCT `pair` FROM `accounts` ; ");
mysql_query("DELETE FROM `accounts`; ");
mysql_query("INSERT INTO `accounts`(`pair`) SELECT (`pair`) FROM `temp`; ");
